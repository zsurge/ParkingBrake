...
void MonitorTask ( void )
{
    if( gRepairMotor.Times == 0 && gRepairMotor.Direction == POS_DW)
    {
        Uart_Print(1,0);
        gRepairMotor.Times = 2;
    }
...
//            //这里可以设置起始点为39，第一个落点如果不是39，记载一个偏移量 for(i=0;i<calccount;i++)offset+=offset;
          //if ( gRepairMotor.AverageValue >= THREE_SPRING_LOWER_LIMIT && SpeRinN > BASIC_SPRING_NUM+BASIC_OFFSET_250MS*gRepairMotor.CalcCounts-2 ) //4米杆
//            if(gRepairMotor.OffsetSum >= THREE_SPRING_LOWER_LIMIT && (SpeRinN >= TWO_SPRING_MIN_NUM))
//            {
//                gRepairMotor.OutFlag = 1;
//                //打印当前测速环值
//                Uart_Print(3,3);
