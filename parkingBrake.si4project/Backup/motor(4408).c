...
void MonitorTask ( void )
{
    if(gRepairMotor.Times == 0 && gRepairMotor.ExFlag == 0 && gRepairMotor.Direction == POS_DW) //测速环转动才开始进来
    {
        ...
        {     
            //记录第一次落点的位置，并做为基准，计算之后的数据
            if ( gRepairMotor.CalcCounts == 0 )
            {
                gRepairMotor.BasicSpringNum = SpeRinN;
            }
            
            gRepairMotor.CalcCounts++;

            if(SpeRinN > gRepairMotor.BasicSpringNum + BASIC_OFFSET_250MS*(gRepairMotor.CalcCounts+1))
            {
                //直接抬闸，报警
...
//                Uart_Print(3,25);
            ...
            else if(SpeRinN > gRepairMotor.BasicSpringNum + BASIC_OFFSET_250MS*gRepairMotor.CalcCounts)
            {
                gRepairMotor.FlagValue += 1;//这里应该不会出现，如果出现就是大问题
            }
            else if(SpeRinN > (gRepairMotor.BasicSpringNum + BASIC_OFFSET_250MS*(gRepairMotor.CalcCounts-1)))
            {
                 gRepairMotor.OffsetSum += SpeRinN-(gRepairMotor.BasicSpringNum + BASIC_OFFSET_250MS*(gRepairMotor.CalcCounts-1));
            }
            else
            {
                ;
            ...
            //同样的，若是圈数走的太多，就直接报错
            if (gRepairMotor.FlagValue >= 2 || gRepairMotor.OffsetSum > TWO_SPRINT_LOWER_LIMIT) //不需完全走完判定，可以直接认为是有问题的
            {
                //直接抬闸，报警
                gRepairMotor.OutFlag = 1;
                Event(ITASK_DG_UP);
//                Uart_Print(3,5);
            ...
            if(gRepairMotor.OffsetSum >= THREE_SPRING_LOWER_LIMIT && (SpeRinN > (gRepairMotor.BasicSpringNum + BASIC_OFFSET_250MS*gRepairMotor.CalcCounts -2)))
            {
                gRepairMotor.OutFlag = 1;
                //打印当前测速环值
//                Uart_Print(3,3);
